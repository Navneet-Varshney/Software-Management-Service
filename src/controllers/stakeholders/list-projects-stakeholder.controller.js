// controllers/stakeholders/list-projects-stakeholder.controller.js

const { listProjectsStakeholderService } = require("@services/projects/get-project.service");
...
/**
 * All valid status / phase values a stakeholder may filter by.
 * Deleted / archived projects are never returned.
 */
const VALID_STATUSES = Object.values(ProjectStatus);
const VALID_PHASES   = Object.values(Phases);

/**
 * Controller: Get Project List – Stakeholder View
 *
 * @route  GET /software-management-service/api/v1/admin/stakeholder/list-projects
 ...
 *
 * Only public (non-deleted) fields are returned.
 * Field selection is limited to the safe whitelist (same as client view).
 *
 * @returns {200} Paginated project list (restricted fields)
 * @returns {400} Invalid filter values
 * @returns {500} Internal server error
 */
const listProjectsStakeholderController = async (req, res) => {
  try {
    const { projectStatus, currentPhase } = req.query;

    // ── Validate enum filters ─────────────────────────────────────────────────
    if (projectStatus && !VALID_STATUSES.includes(projectStatus)) {
      logWithTime(`❌ [listProjectsStakeholderController] Invalid projectStatus filter | ${getLogIdentifiers(req)}`);
    ...
    if (currentPhase && !VALID_PHASES.includes(currentPhase)) {
      logWithTime(`❌ [listProjectsStakeholderController] Invalid currentPhase filter | ${getLogIdentifiers(req)}`);
      return throwBadRequestError(
        res,
    ...
    const result = await listProjectsStakeholderService(filters, { page, limit, selectFields });

    if (!result.success) {
      logWithTime(`❌ [listProjectsStakeholderController] ${result.message} | ${getLogIdentifiers(req)}`);
      return throwSpecificInternalServerError(res, result.message);
    }

    logWithTime(`✅ [listProjectsStakeholderController] Projects list fetched successfully | ${getLogIdentifiers(req)}`);
    return sendProjectsListFetchedSuccess(res, result.projects, result.total, result.page, result.totalPages);
  } catch (error) {
    logWithTime(`❌ [listProjectsStakeholderController] Unexpected error: ${error.message} | ${getLogIdentifiers(req)}`);
    return throwInternalServerError(res, error);
  }
};

module.exports = { listProjectsStakeholderController };
